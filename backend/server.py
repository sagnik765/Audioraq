...
AI_STUDIO_STAGE_LABELS = {
    ...
    "quality_review": "AI Agents Review",
    "publish": "Publish",
}

try:
    AUDIORAQ_ORIGINALS_MIN_QUALITY_SCORE = float(os.environ.get("AUDIORAQ_ORIGINALS_MIN_QUALITY_SCORE", 90.0))
except (TypeError, ValueError):
...
def quality_split_sentences(text: str) -> List[str]:
...
def quality_text_features(text: str, generation: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    words = re.findall(r"[a-zA-Z][a-zA-Z']+", text or "")
    lowered_words = [word.lower() for word in words]
    sentences = quality_split_sentences(text)
...
def quality_gan_inspired_discriminator(text: str, generation: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    features = quality_text_features(text, generation=generation)
    score = 24.0
...
def quality_rag_safety_review(text: str) -> Dict[str, Any]:
    matched = []
    lowered = text or ""
    for doc in QUALITY_RAG_SAFETY_KB:
...
def quality_rlaif_self_feedback(gan_review: Dict[str, Any], rag_review: Dict[str, Any], generation: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def quality_scorecard_item(score: float, note: str) -> Dict[str, Any]:
    return {
        "score": round(max(0, min(100, score)), 1),
        "note": note,
    }
...
def build_quality_scorecard(
    ...
    scorecard = {
        "hook_strength": quality_scorecard_item(hook_score, "Cold-open clarity, listener promise, and avoidance of generic setup."),
        "dialogue_realism": quality_scorecard_item(dialogue_score, "Speaker variation, turn-taking, and question-led pacing."),
        "specificity": quality_scorecard_item(specificity_score, "Concrete examples, named constraints, and low filler density."),
        "structure": quality_scorecard_item(structure_score, "Outline completeness and section-to-beat cohesion."),
        "factual_safety": quality_scorecard_item(factual_score, "RAG-style safety retrieval and claim-risk screening."),
        "audio_readiness": quality_scorecard_item(audio_score, "Voice-ready turns, production notes, render preparedness, and measured audio clarity."),
    }
    if voice_clarity_score is not None:
        scorecard["voice_clarity"] = quality_scorecard_item(voice_clarity_score, voice_clarity.get("summary") or "Measured signal clarity from rendered or uploaded audio.")
    if resonance_score is not None:
        scorecard["voice_resonance"] = quality_scorecard_item(
            resonance_score,
            "Warm low-mid presence, stable timbre, and non-fatiguing brightness proxy.",
        )
    if articulation_score is not None:
        scorecard["voice_articulation"] = quality_scorecard_item(
            articulation_score,
    ...
    if voice_review and voice_review.get("listenability_score") is not None:
        scorecard["podcast_voice_listenability"] = quality_scorecard_item(
            float(voice_review.get("listenability_score") or 0),
            voice_review.get("summary") or "Long-form podcast voice listenability.",
        )
    scorecard["publish_readiness"] = quality_scorecard_item(readiness_score, "Combined AI Agents signal for whether this can move toward publishing.")
    return scorecard
...
def evaluate_quality_review(
    ...
    review_text = quality_build_review_text(title, description, generation=generation, media_analysis=media_analysis)
    gan_review = quality_gan_inspired_discriminator(review_text, generation=generation)
    rag_review = quality_rag_safety_review(review_text)
    rlaif_feedback = quality_rlaif_self_feedback(gan_review, rag_review, generation=generation)
...
def merge_quality_review_into_moderation(moderation: Dict[str, Any], quality_agent: Dict[str, Any]) -> Dict[str, Any]:
    ...
    merged["provider"] = f"{merged.get('provider') or 'heuristic'}+quality-review"
...
async def revise_ai_generation_with_quality_feedback(
    brief: Dict[str, Any],
    show: Dict[str, Any],
...
def build_ai_studio_stage_state(
    ...
    if quality_review:
        review_status = quality_review.get("status") or "pass"
        ...
        state = update_ai_studio_stage_state(
            ...
            "quality_review",
            stage_status,
            quality_review.get("summary", "AI Agents review complete."),
        )
...
def resolve_ai_studio_active_stage(
    ...
    if published_episode_id:
        return "publish"
    if quality_review and quality_review.get("status") in {"blocked", "revise"}:
...
def build_ai_studio_project_doc(
    ...
    return {
        ...
        "generation": generation or {},
        "show_bible": build_ai_studio_show_bible(show, intake),
        "artifacts": build_ai_studio_artifacts(intake, show, generation, quality_review),
        "quality_review": quality_review or {},
        "stage_state": build_ai_studio_stage_state(intake, generation, quality_review),
        "active_stage": resolve_ai_studio_active_stage(generation, quality_review),
        "status": "draft",
        "media_policy": {
...
async def get_ai_studio_status(request: Request):
    ...
    if get_storage_backend() == "emergent":
        warnings.append("Media storage is still using the legacy object store. Set STORAGE_BACKEND=local after migrating existing media.")
    ...
    return {
        ...
        "quality_gate": {
            ...
            "publishes_after_quality_review": True,
...
async def update_ai_studio_project(project_id: str, req: UpdateAIStudioProjectRequest, request: Request):
    ...
    if req.intake is not None:
        ...
        updates["artifacts"] = build_ai_studio_artifacts(intake, show, project.get("generation") or {}, project.get("quality_review") or {})
        updates["stage_state"] = build_ai_studio_stage_state(intake, project.get("generation") or {}, project.get("quality_review") or {}, project.get("published_episode_id") or "")
...
async def generate_ai_podcast_draft(req: GenerateAIPodcastDraftRequest, request: Request):
    ...
    quality_iterations = [{"iteration": 1, "event": "initial_review", "review": quality_review}]
    revised_generation = await revise_ai_generation_with_quality_feedback(intake, show, generation, quality_review)
    if revised_generation:
        revised_quality_review = evaluate_quality_review(
        ...
        revision_improved = revised_quality_review.get("quality_score", 0) >= quality_review.get("quality_score", 0)
        revision_safe = revised_quality_review.get("rag_safety", {}).get("status") != "blocked"
        quality_iterations.append(
            {
                ...
                "review": revised_quality_review,
            }
        )
        if revision_improved and revision_safe:
            generation = revised_generation
            quality_review = revised_quality_review
    ...
    draft_doc = {
        ...
        "quality_status": quality_review.get("status", "pass"),
...
async def get_show_thumbnail(show_id: str):
    ...
    try:
        ...
        logger.warning(f"Show thumbnail missing for {show_id}; serving generated fallback")
        return generated_thumbnail_response(
            show.get("title", "Audioraq Show"),
            show.get("podcaster_name", ""),
            show.get("category", DEFAULT_SHOW_CATEGORY),
...
async def upload_podcast(
    ...
    quality_agent = evaluate_quality_review(
        title,
        description,
        generation=ai_draft.get("generation") if ai_draft else None,
        media_analysis=media_analysis,
    ...
    moderation = merge_quality_review_into_moderation(moderation, quality_agent)
...
async def stream_podcast(podcast_id: str, request: Request):
    ...
    try:
        ...
        response = stream_stored_object(podcast["media_path"], content_type, request, filename)
        ...
        logger.warning(f"Episode media missing for {podcast_id}")
        raise HTTPException(status_code=404, detail="Episode media is not available")
    except Exception as e:
...
async def get_thumbnail(podcast_id: str, request: Request):
    ...
    try:
        ...
        logger.warning(f"Episode thumbnail missing for {podcast_id}; serving generated fallback")
        return generated_thumbnail_response(
            podcast.get("title", "Audioraq Episode"),
            podcast.get("show_title", podcast.get("podcaster_name", "")),
            podcast.get("category", DEFAULT_SHOW_CATEGORY),
